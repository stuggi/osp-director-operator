...
func SyncGit(
	...
	log.Info(fmt.Sprintf("BOO repoURL %+v", repoURL))
	log.Info(fmt.Sprintf("BOO repoAddr %s", repoAddr))
	log.Info(fmt.Sprintf("BOO repo user %s", repoURL.User.Username()))
	log.Info(fmt.Sprintf("BOO repo Path %s", repoURL.Path))
	log.Info(fmt.Sprintf("BOO repo URL %s", repoURL.String()))
	log.Info(fmt.Sprintf("BOO pkey %s", string(pkey)))
	...
	log.Info(fmt.Sprintf("BOO auths %v", crypto_ssh.PublicKeys(signer)))
		...
		if ref.Name().IsBranch() {
			...
			refs.ForEach(func(ref *plumbing.Reference) error {
				if ref.Type() == plumbing.HashReference {
					log.Info(fmt.Sprintf("reference: %v", ref))
				}
